...
def process_data(file_name):
    ...
    ## Jinny: split() 한계로 구두점 붙어 있는 경우 ex)fire, / fire , 구두점 같이 단어로 split 해버림 
    ##따라서 전처리 할 때 구두점 먼저 제거 하고 시작 하는 걸로 ..ㅇㅇ 
    
    ## Jinny: remove puctutaion: str.maketrans(변환할 문자, 변환될 문자,삭제할 문자)
    #contents_no_punct = contents_lower.translate(str.maketrans('','',string.punctuation))
    ## Jinny근데,, 이렇게 하면 특수문자 다 날아가서 안됨 I'm 같은거 문제 생김 
    
    words = re.findall (r'\w+', contents_lower)
    #JInny: 이렇게 하면 축약형 살릴 수 있음 
...
def delete_letter(word, verbose=False):
    ...
    #Jinny: list comprehension [expression for item in iterable if condition], 반복문&조건문 같이 
    split_l = [(word[:i], word[i:]) for i in range(len(word) + 1)]
    #print(split_l)
    
    for left, right in split_l:
        #Jinny: 삭제를 따로 해준다는 개념보다, right 에서 1부터 가져 오면 그게 삭제인 것.. 
        # 근데 이렇게 하면 두 글자일 때 issue, 그래서 if rifht: 조건 넣어줘서, 두 글자 초과일때만 delete 생성하도록
        #if right: 이렇게 줘도 됨=> right가 비어있지 않으면 true 
        
        if len(right) != 0:
            delete = left + right[1:]
            delete_l.append(delete)
...
def switch_letter(word, verbose=False):
    ...
    for a,b in split_l:
        if len(b)>1: #두 글자 방지
            #JInny: 이런 경우 out of index 오류 남, 이 문제 같은 경우 두 글자에서는 스위치가 일어나면 안되게끔 정의 한듯.. 
            #JInny_Q: 인접한 두 글자라면 at => ta 이렇게 되는게 맞는거 같은디
            #switch = a + b[1] + b[0] + b[2]
            
            switch = a + b[1] + b[0] + b[2:] #인접한 letter만 바꾸는 거 
...
# test # 2
#Jinny_Q 아니 뭐야 이경우에 0이 나와야 되는거 아님???????????????????
...
def replace_letter(word, verbose=False):
    ...
    for left, right in split_l:
        for c in letters: 
            new_words = left + c + right[1:] #c가 right[0] 을 대체 할 거니까
            
            #여기서  Remove the original input letter from the output 해줘야 함,중복 방지
            # 길이로만 비교하면 한계 있음 and 조건 추가 
            if len(new_words) == len(word) and new_words != word:
                replace_list.append(new_words)
...
def edit_one_letter(word, allow_switches = True):
    ...
    # Combine the sets
    edit_one_set.update(delete)
    edit_one_set.update(insert)
    edit_one_set.update(replace)
    if allow_switches:
        edit_one_set.update(switch)
    ### END CODE HERE ###
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    if word in vocab:
        ...
        # Get suggestions from edit_one_letter
        one_edits = edit_one_letter(word)
        suggestions += [(w, probs[w]) for w in one_edits if w in vocab]
        
        # Get suggestions from edit_two_letters if needed
        if len(suggestions) < n:
            two_edits = edit_two_letters(word)
            suggestions += [(w, probs[w]) for w in two_edits if w in vocab and w not in dict(suggestions)]
                    
    #Step 2: determine probability of suggestions
    best_words = {w: probs.get(w, 0) for w, _ in suggestions}
    
    #Step 3: Get all your best words and return the most probable top n_suggested words as n_best
    
    suggestions = sorted(suggestions, key=lambda x: x[1], reverse=True)
    n_best = suggestions[:n]
...
def min_edit_distance(source, target, ins_cost = 1, del_cost = 1, rep_cost = 2):
    ...
    for row in range(1, m+1):
        ...
        for col in range(1, n+1):
            ...
            D[row,col] =  min(D[row-1,col] + del_cost,               # Cost of deletion
